...
```
fn_item := [attrs] ["pub"] "fn" IDENT ...
```

```rust
pub fn open_the_gate() { raw!("setblock 0 64 0 minecraft:air"); }
```

- **意味は 1 つだけ: 「パックの外から呼ばれうる」。** release のデッドコード除去
  （[§6.34](#634-デッドコード除去--確定m9)）は function タグから辿れない関数を落とすので、
  `/function` を手で打つ・コマンドブロックから呼ぶ・他のデータパックから呼ぶ関数には
  そう書いてもらう
- **モジュールの可視性ではない。** `mod` / `use` はまだ無く、
  1 つのパックは 1 つのコンパイル単位なので、隠す相手がいない
- 付けられるのは関数だけ。`struct` や `enum` に付けてもまだ意味が無いので拒否する
- **`#[ctx]` は拒否しない。** タグと違い、外からの呼び出しが実行者を持っていることは
  ありうる（コマンドブロックの `execute as ... run function`）。
  実行者なしで呼ばれたときに黙って何もしないのはバニラどおりで、
  debug ビルドのガードがそれを報告する（[§6.15](#615-実行コンテキスト)）
...
**この結果、release では `#[tick]` / `#[load]` / `pub` の付かない関数が出力から消える。**
`/function myns:foo` を手で打って呼ぶ、あるいは他のパックから呼ぶつもりの関数には
**`pub` を付ける**（[§3.26](#326-pub--確定m11)）。それが「外から入る口」の宣言で、
根に加わる。
