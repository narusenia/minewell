...
| 節 | 内容 | タスク |
|---|---|---|
| 3.2 | `let`、式、演算子の優先順位 | M2 |
| 3.3 | `if` / `else` / `while` / `loop` / `break` / `continue` / `return` | M3 |
| 3.4 | 関数の引数・戻り値・呼び出し | M4 |
| 3.5 | `as` / `at` / `for`、`#[ctx]` | M5 |
| 3.6 | `struct` / `enum` / `match` / ジェネリクス / `impl` | M7 |
| 3.7 | `mod` / `use` / `pub` / `extern fn` | M1〜M4 のどこかで最小限、完全版は M7 |

---

## 4. 意味論 — *未定*

型検査規則、名前解決規則、`#[ctx]` の伝播規則。M2 以降。

## 5. 型 — *未定*

`i32` / `bool` / `fix<S>` / storage 専用型 / `String` / `Vec<T>` / ドメイン型。
要件定義 §4 に決定はあるが、規則としての記述は M2 と M8。

## 6. lowering — *未定*

各構文から mcfunction への写像。M2 以降、タスクごとに書き足す。
生成コマンド数は `tinymcf` の計測 API で検証する（[`../crates/tinymcf/SPEC.md`](../crates/tinymcf/SPEC.md) §5）。
