...
- ~~ブロックの NBT は v1 では持たない。座標のモデルが要る~~
  **M10 で入れた。** 座標のモデル（`positioned`・`^`・`if block`）と一緒に開いた。
  `#[block] struct` と `T::of(pos!(..))` で、エンティティのビューと同じ形（仕様 §3.25）
...
- 実行時値の補間は、**その行だけを本体に持つマクロ関数**（`$(v)` を含み
  `function ... with storage` で呼ぶ必要がある関数）を生成して、そこへ落とす
- `{` はつねに補間の始まり。`{` `}` の文字そのものは `{{` `}}` と二重に書く。
  生コマンドは NBT の `{...}` を書くことが多いが、「名前として解決できなければ文字面」に
  すると綴り違いが黙って通る（設計原則 2）

**昇格は補助関数に閉じる。** マクロ性は「呼ばれ方」の属性であり、`$` の行を持つのは
生成した補助関数だけ。`raw!` を書いた関数も、その呼び出し側も、マクロ関数にはならない。
コストは実行時の値 1 つにつきマーシャル 1 + 呼び出し 1 + マクロ行 1 で、局所。

> **当初案（関数そのものを昇格させる）を採らなかった理由 — M9-3。**
> 元はこの節に「`#[tick]` / `#[load]` の関数がマクロ関数になったらコンパイルエラー」と
> 書いていた。function tag は引数なしで呼ぶため、実行時に沈黙して失敗するからである。
> だが関数そのものを昇格させる案は**局所変数を補間できない**: `let x = f();` の `x` は
> 呼び出し側が知らない値なので、`mw.args` に書ける者がいない。補間できるのは仮引数だけに
> なってしまう。補助関数へ落とせばこの制限が消え、タグの関数でも実行時補間が書けるので、
> **`#[tick]` / `#[load]` の検査は不要になった**（仕様 §6.31）。
...
> **M9-4 で入れた範囲。** 連結・自動変換・色・装飾までを実装した。
> `fix<S>` は score コンポーネントにしない — バニラの score には倍率が無く、
> スケール後の整数がそのまま出てしまう（`raw!` の補間と同じ判断。仕様 §3.21・§3.22）。
> `.on_click` / `.on_hover` / `translate!` は未実装。クリックとホバーの JSON は
> 1.21.5 で形が変わっており、**版差は toolchain が持つもの**で、コンパイラに
> 埋め込むものではない。toolchain がイベントの形を記述できるようになってから入れる。
> 補間できるのは束縛そのものだけで、`m.hp` のような場所は先に `let` で受ける。
...
**エディタの構文ハイライトは対象外ではない。** ここで外しているのは LSP
（診断・補完・定義ジャンプを話す常駐プロセス）であって、tree-sitter / TextMate の
grammar はコンパイラに一切依存せず、v1 の中で用意する（計画 X-6）。
