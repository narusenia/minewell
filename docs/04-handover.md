...
2026-08-25 時点。**78 / 91 タスク完了、テスト 504、コミット 80。**
M0〜M8 完了。次は M9（仕上げ: `Option`・マクロ昇格・`text!`・release 最適化・CLI）。
...
| 4 | [`02-spec.md`](./02-spec.md) | 文法・型規則・lowering。M8 の範囲まで確定 |
...
```rust
struct Arena { #[nbt(rename = "Radius")] radius: f64, name: String }
...
動かないもの: `Option`（`#[nbt(optional)]` 含む）・`enum` のジェネリクス・`text!`・
...
エンティティ・ブロックの NBT の読み取り（失敗しうるので `Option` と一緒に入れる = M9-1）。
...
1. **M9-1（`Option<T>`）が先。** `enum` のジェネリクスがここで入り、
   `#[nbt(optional)]` とエンティティ NBT の読み取り（`get_data`）がその上に乗る。
   **M8-4 でエンティティ NBT を入れなかったのはこれが理由** — 読み取りは失敗しうるので、
   `Option` の無い状態で構文だけ作ると後で書き方が 2 つ残る。
   tinymcf 側に `data` の entity / block 対象を足すのもここ
2. **M9-3（`raw!` のマクロ昇格）は M9-4（`text!`）の前。** 実行時値の補間という
   同じ仕組みを使う。文字列の連結・実行時比較で**マクロ昇格の型はもうできている**
   （`mw.args` に書いて `function ... with storage` で呼ぶ。仕様 §6.27）
3. **M9-5〜M9-7（最適化）は最後。** 意味を変えない変換なので、
...
| **`#[nbt(optional)]` / `Option<T>`** | `enum` のジェネリクスが要る | M9-1 |
